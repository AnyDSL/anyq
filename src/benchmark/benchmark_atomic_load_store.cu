...
int main(int argc, char** argv)
{
	try
	{
		...
		for (int occupancy : {1, 5, 10, 25, 50, 75, 100})
		{
			...
			std::cout << "\n   [1024 : 1] <  2:"sv;
			kernels<N, strided_access<1024, 1>, lower_lanes< 2>>(try_relaxed_atomics, print_results);
			std::cout << "\n   [1024 : 1] <  4:"sv;
			kernels<N, strided_access<1024, 1>, lower_lanes< 4>>(try_relaxed_atomics, print_results);
			std::cout << "\n   [1024 : 1] <  8:"sv;
			kernels<N, strided_access<1024, 1>, lower_lanes< 8>>(try_relaxed_atomics, print_results);
			std::cout << "\n   [1024 : 1] < 16:"sv;
			kernels<N, strided_access<1024, 1>, lower_lanes<16>>(try_relaxed_atomics, print_results);
			std::cout << "\n   [1024 : 1] < 32:"sv;
			kernels<N, strided_access<1024, 1>, lower_lanes<32>>(try_relaxed_atomics, print_results);
			std::cout << '\n';

			std::cout << "\n   [1024 : 1] %  1:"sv;
			kernels<N, strided_access<1024, 1>, skip_lanes< 1>>(try_relaxed_atomics, print_results);
			std::cout << "\n   [1024 : 1] %  2:"sv;
			kernels<N, strided_access<1024, 1>, skip_lanes< 2>>(try_relaxed_atomics, print_results);
			std::cout << "\n   [1024 : 1] %  4:"sv;
			kernels<N, strided_access<1024, 1>, skip_lanes< 4>>(try_relaxed_atomics, print_results);
			std::cout << "\n   [1024 : 1] %  8:"sv;
			kernels<N, strided_access<1024, 1>, skip_lanes< 8>>(try_relaxed_atomics, print_results);
			std::cout << "\n   [1024 : 1] % 16:"sv;
			kernels<N, strided_access<1024, 1>, skip_lanes<16>>(try_relaxed_atomics, print_results);
			std::cout << "\n   [1024 : 1] % 32:"sv;
			kernels<N, strided_access<1024, 1>, skip_lanes<32>>(try_relaxed_atomics, print_results);
			std::cout << '\n';
