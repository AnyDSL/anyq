...
class Plot {
	constructor(svg, y_axis, y_scale, lineType, margin) {
		this.svg = svg;
		this.width = () => 400; //svg.width();
		this.height = () => 300; //svg.height();
		this.margin = margin;
	...
	update(y_domain) {
		...
		this.axis.x.call(d3.axisBottom(this.x).ticks(6))
				   .attr("transform", `translate(0, ${this.y(this.y_min)})`);
		this.axis.y.call(d3.axisLeft(this.y).ticks(6, this.y_tickFormat))
...
{
	let svg = $('<svg viewBox="0 0 400 300" class="plot-graph" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"></svg>').appendTo(plotElem);
